"""Core month-by-month rules for the Personal Finance Game.

The engine is deliberately independent of a UI. A caller supplies a player's
monthly fun allocations and may inject a seeded ``random.Random`` instance for
replayable games and tests.
"""
...
class GameConfig:
    ...
    min_fun: float = 0.0
    max_fun: float = 400.0
    fun_baseline: float = 200.0
    missed_rent_happiness_loss: float = 60.0
    missed_other_expenses_happiness_loss: float = 45.0
    food_shortage_happiness_loss: float = 80.0
    no_fun_happiness_loss: float = 20.0
...
def _validate_config(config: GameConfig) -> None:
    ...
    if config.fun_baseline < config.min_fun or config.fun_baseline > config.max_fun:
        raise ValueError("fun_baseline must be within the fun range")
...
def run_simulation(
    ...
    *,
    ...
    """Run one player's 12-month simulation.

    ``fun_allocations`` is the player's input for each month. Required bills are
    paid in order (rent, then other expenses); unpaid amounts never create
    negative savings and instead trigger happiness penalties.
    """
    ...
    for month, requested_fun in enumerate(fun_allocations, start=1):
        month_start_savings = state.savings
        ...
        if (
            ...
            if not job_lost and rng.random() < config.fortune.job_loss_chance:
                ...
                bad_fortune = "job_loss"

        rent_paid = min(state.savings, config.rent)
        state.savings -= rent_paid
        missed_rent = rent_paid < config.rent
        other_paid = min(state.savings, config.other_expenses)
        state.savings -= other_paid
        missed_other = other_paid < config.other_expenses
        food_shortage = missed_other

        available_for_fun = min(state.savings, requested_fun)
        state.savings -= available_for_fun
        happiness_change = available_for_fun - config.fun_baseline
        ...
        if food_shortage:
            happiness_change -= config.food_shortage_happiness_loss
        if bad_fortune:
        ...
        results.append(
            MonthResult(
                ...
                income=income,
                rent_paid=rent_paid,
                other_expenses_paid=other_paid,
                fun_spending=available_for_fun,
                gifts=gifts,
                bad_fortune=bad_fortune,
                job_lost=job_lost,
                ...
                missed_other_expenses=missed_other,
...
def rank_simulations(results: Sequence[SimulationResult]) -> list[SimulationResult]:
    """Rank players by ending savings, then average happiness."""

    return sorted(results, key=lambda result: (result.ending_savings, result.average_happiness), reverse=True)
