...
test('student builds twelve budgets, downloads, reopens and submits',async({page})=>{
  ...
  await expect(page.locator('#field-rent')).toHaveAttribute('readonly','');
  for(const key of ['food','utilities','transport'])await expect(page.locator('#field-'+key)).toHaveValue('25');
  ...
  const values={food:250,utilities:50,transport:50,personal:50,fun:100,investment:200,savings:300};
  ...
  page.once('dialog',dialog=>dialog.accept());
  await page.locator('#reset-expenses').click();
  await page.reload();
  await expect(page.locator('#field-food')).toHaveValue('25');
  await expect(page.locator('#field-fun')).toHaveValue('100');
  await page.getByRole('button',{name:'Edit month 12',exact:true}).click();
  await expect(page.locator('#field-transport')).toHaveValue('25');
  await expect(page.locator('#field-personal')).toHaveValue('50');
  await expect(page.locator('#month-notes')).toHaveValue('Save for emergencies.');
